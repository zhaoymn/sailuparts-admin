...
export async function getPaintingById(id: string) {
  try {
    ...
    if (!painting.all_imageIds || painting.all_imageIds.length !== painting.all_images.length) {
      painting.all_imageIds = new Array(painting.all_images.length).fill('');
    }
    if (!painting.all_imageIds_150 || painting.all_imageIds_150.length !== painting.all_images_150.length) {
      painting.all_imageIds_150 = new Array(painting.all_images_150.length).fill('');
    }
    if (!painting.all_imageIds_300 || painting.all_imageIds_300.length !== painting.all_images_300.length) {
      painting.all_imageIds_300 = new Array(painting.all_images_300.length).fill('');
    }
    if (!painting.all_imageIds_1000 || painting.all_imageIds_1000.length !== painting.all_images_1000.length) {
      painting.all_imageIds_1000 = new Array(painting.all_images_1000.length).fill('');
    }
...
export async function updatePaintingItem(paintingId: string, data: string) {
  try {
    ...
    if (painting.artist_id !== paintingData.artist_id) {
      ...
      if (oldArtist) {
        oldArtist.paintings = oldArtist.paintings.filter((painting: any) => painting.toString() !== painting._id.toString());
        await oldArtist.save();
      }
      const newArtist = await Artist.findOne({ artist_id: paintingData.artist_id });
      if (newArtist) {
    ...
    painting.artist = await Artist.findOne({ artist_id: paintingData.artist_id });
    ...
    painting.awards = paintingData.awards.filter((award: string) => award.trim() !== "");
    painting.awards_chinese = paintingData.awards_chinese.filter((award: string) => award.trim() !== "");
    painting.tags = paintingData.tags.filter((tag: string) => tag.trim() !== "");
...
export async function addImageToPainting(paintingId: string, result: AddImageToPaintingParams) {
  try {
    await connectToDatabase();
    const painting = await Painting.findOne({ painting_id: paintingId });
    if (!painting) throw new Error('Painting not found');
...
export async function removeImageFromPainting(paintingId: string, imageUrl: string) {
  try {
    await connectToDatabase();
    const painting = await Painting.findOne({ painting_id: paintingId });
...
export async function changeImageRenderedStatus(status: boolean, index:number, paintingId: string) {
  try {
    await connectToDatabase();
    const painting = await Painting.findOne({ painting_id: paintingId });
    if (!painting) throw new Error('Painting not found');
...
export const paintingIdExists = async (paintingId: string): Promise<boolean> => {
  try {
    await connectToDatabase();
    const painting = await Painting.findOne({ painting_id: paintingId });
    return !!painting;
...
export const copyPainting = async (original_id: string, newPaintingId: string): Promise<string> => {
  try {
    await connectToDatabase();
